...
func (o *ReplaySessionURL) Build() (*url.URL, error) {
	...
	var _path = "/api/sessions/{session_id}/replay"

	sessionID := swag.FormatInt64(o.SessionID)
	if sessionID != "" {
		_path = strings.Replace(_path, "{session_id}", sessionID, -1)
	} else {
		return nil, errors.New("SessionID is required on ReplaySessionURL")
	}
...
func (o *ReplaySessionURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on ReplaySessionURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on ReplaySessionURL")
	}
